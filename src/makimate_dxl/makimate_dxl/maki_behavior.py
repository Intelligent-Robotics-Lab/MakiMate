...
class MakiBehavior(Node):
    ...
    # - slower transitions (less jitter)
    # - stronger eye stabilization
    # - built-in blink every 6 seconds
    # ==========================================
    def start_big_shake_no(self):

        state = {
            "yaw": 0.0,
            "target": 10.0,     # slightly smaller amplitude
            "dwell": 0,
            "eye_yaw": 0.0,
            "blink_t": 0,
        }
        ...
        def step_timer():
            # --- Neck motion ---
            alpha = 0.15   # slower easing = less jitter
            yaw = state["yaw"] + alpha * (state["target"] - state["yaw"])
            state["yaw"] = yaw

            # Flip target after dwelling at the ends
            if abs(state["target"] - yaw) < 0.8:
                state["dwell"] += 1
                if state["dwell"] > 8:
            ...
            # --- Eye stabilization ---
            # stronger compensation gain:
            target_eye_yaw = -1.3 * yaw

            # smooth the eyes slightly (no jitter)
            eye_alpha = 0.25
            ...
            state["blink_t"] += 1
            blink_phase = state["blink_t"] % 120  # 120 * 0.05 = 6 seconds

            if blink_phase < 6:         # closing
                lid_left = -14.0
                lid_right = 18.0
            elif blink_phase < 12:      # opening
                lid_left = 20.0
                lid_right = -20.0
            else:                        # neutral eyes
                lid_left = 8.0
            ...
            # Send the final combined command
            arr = [yaw, 0.0, 0.0, eye_yaw, lid_left, lid_right]
            self.send(arr)


        # slower loop rate → smoother movement
        t = self.create_timer(0.05, step_timer)
        self._timers.append(t)
    ...
    def on_face_pos(self, msg: Float64MultiArray):
        ...
        # Map face x/y into neck yaw & pitch
        target_yaw = 25.0 * x      # left/right
        target_pitch = 12.0 * y    # up/down

        # Eyes wide open while tracking
        arr = [target_yaw, target_pitch, 0.0, 0.0, 20.0, -20.0]
        self.send(arr)
